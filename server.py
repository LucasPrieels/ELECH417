...
clients = {} # List of clients connected with their username and their connections
current_port = 0
...
def signup(client_connection):
    ...
    if usr in credentials: # User already exists
        ...
        client_connection.send(str.encode("1"))
        print("Signup successful")
        f = open("credentials.txt", "a")
        f.write(usr + " " + pswd + "\n")
        return usr
    
def login(client_connection):
    credentials = parse_credentials_file()
    ...
    if usr not in credentials:
        ...
        client_connection.send(str.encode("2"))
